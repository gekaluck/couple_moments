  ...
  return (
    <section id="idea-comments" className="surface p-6">
      ...
      {!isComposerOpen ? (
        <button
          type="button"
          className="mt-4 w-full rounded-2xl border border-dashed border-[var(--panel-border)] bg-white/65 px-4 py-3 text-left text-sm text-[var(--text-muted)] transition hover:border-amber-200 hover:bg-amber-50/40"
      ...
      {isComposerOpen ? (
        <form ref={formRef} className="mt-4 flex flex-col gap-3" action={handleSubmit}>
          ...
          <div className="flex justify-end">
            <button
              className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 px-4 py-2 text-xs font-semibold text-white shadow-[var(--shadow-md)] transition hover:shadow-[var(--shadow-lg)] disabled:opacity-50"
              type="submit"
              disabled={isPending}
            >
