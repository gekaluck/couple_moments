...
export default function MobileAgendaView({
  ...
  const visibleDays = showAll
    ? daysWithContent
    : daysWithContent.slice(0, DEFAULT_DAYS_SHOWN);
  const hasMore = daysWithContent.length > DEFAULT_DAYS_SHOWN;
  const hiddenCount = daysWithContent.length - DEFAULT_DAYS_SHOWN;
